...
class eDMD_MultiStep:
    ...
    def project_to_x(self, zhist: jnp.ndarray) -> jnp.ndarray:
        if zhist.ndim == 1:
            return zhist[:self.nx]
        elif zhist.ndim == 2:
            return zhist[:, :self.nx]
    ...
    def rollout(self, A, B, zs, us, H):
        def scan_step(carry, inputs):
            z_prev = carry
            u_t = inputs
            z_next = A @ z_prev + B @ u_t
            return z_next, z_next

        def rollout_single(z0, us_seq):
            _, zs_out = jax.lax.scan(scan_step, z0, us_seq[:H])
            return jnp.concatenate([z0[None, :], zs_out], axis=0)

        z_preds = jax.vmap(rollout_single)(zs[:, 0, :], us)
        return z_preds

    def multi_step_loss(self, params, zhist, uhist, H):
        A, B = params
        z_preds = self.rollout(A, B, zhist, uhist, H)
        z_targets = zhist[:, :H + 1, :]
        loss = jnp.mean((z_preds - z_targets) ** 2)
        return loss

    def fit(self, zhist: jnp.ndarray, uhist: jnp.ndarray, H: int = 5, steps: int = 1000, lr: float = 1e-2):
        N, T, nz = zhist.shape
        nu = uhist.shape[-1]

        print(f"Multi-step eDMD fit with JAX and Optax:")
        print(f"\t{zhist.shape=}, {uhist.shape=}, horizon H={H}")
        ...
        def step(params, opt_state):
            loss, grads = jax.value_and_grad(self.multi_step_loss)(params, zhist, uhist, H)
        ...
        print(f"Fitted A (shape {self.kpA.shape})")
        print(f"Fitted B (shape {self.kpB.shape})")
